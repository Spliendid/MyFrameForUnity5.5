    ...
    void OnGUI()
    {
        if (GUILayout.Button("LoadAssetbundle"))
        {
            ...
            if (manifestBundle != null)
            {
                ...
                //获取依赖文件列表;  
                string[] cubedepends = manifest.GetAllDependencies("fangzi.data");
                ...
                //加载我们需要的文件;"  
                AssetBundle cubeBundle = AssetBundle.LoadFromFile(Application.streamingAssetsPath + "/Windows/fangzi.data");
                Debug.Log(cubeBundle == null);
                GameObject cube = cubeBundle.LoadAsset("fangzi") as GameObject;
                Debug.Log(cube == null);
                if (cube != null)
                {
                    Instantiate(cube);
